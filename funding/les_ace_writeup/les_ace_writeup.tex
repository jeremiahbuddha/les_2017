...
\newthought{There have been} a number of previous attempts to measure the effectiveness
of leafleting (summarized below). These studies have helped characterize
this largely unexplored landscape, however none of them were designed or
powered appropriately to answer the question of whether leafleting 
college campuses produces vegans at a reliable rate. 
...
The motivation of this study is to quantify the effect of an intervention 
(leafleting). It uses a pre-test / post-test design where each participant 
is evaluated before and after the intervention. Because each subjects 
...
confounding influence of time, which will be addressed a bit later. 
...
quantifying the number of one-week vegans created as the result of leafleting 
...
when analyzing the survey results for one-week vegans. The number of subjects
...
\begin{table*}
  \fontfamily{ppl}\selectfont
  \begin{tabular}{p{1.5cm}p{1.5cm}p{1.5cm}p{1.5cm}p{2 cm}}
    \toprule
...
The inclusion of an equal sized control group will increase
the cost of the study by roughly \$30000. If ACE is willing to 
provide these additional funds, a full control group will be utilized
for the study. If not, than a series of mitigating factors will
be employed to minimize the impact of time on the study. 
...
\begin{table*}
  ...
  \begin{tabular}{lll}
    ...
    Leafleting  & 9000.0 & Vegan Outreach\\                
    \midrule
    Booklets  & 7500.0 & Vegan Outreach\\                                                        
    Stickers  & 22500.0 & ACE\\                                    
    \midrule
    Gift Cards  & 112500.0 & ACE\\                                    
    \bottomrule
  \end{tabular}
  \vskip 1.5em
...
\includepdf[pages={-}]{VO_LES_methods.pdf}
